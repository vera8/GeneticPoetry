...
public class CYK {	
	public static boolean isGrammar(String[] line, String[][] grammar) {
		int n = line.length;
	...
	public static Tree parseTree(String[] line, String[][] grammar) {
		...
		for (int s=0; s<n; s++) {
			for (int i=0; i<termRules.size(); i++) {
				if (termRules.get(i)[1].equals(line[s])) {
					int index = ruleIndices.get(termRules.get(i)[0]);
					//p[0][s][index] = new TreeNode(termRules.get(i)[0], new TreeNode(termRules.get(i)[1]));
